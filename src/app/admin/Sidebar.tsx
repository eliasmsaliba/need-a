...
export function Sidebar({ subRole, adminEmail, navItems, activeSection, onSelect }: SidebarProps) {
  ...
  return (
    <aside className="w-full md:w-[260px] shrink-0 border-b md:border-b-0 md:border-r border-neutral-800 py-6 px-4 flex flex-col gap-5">
      <div className="text-lg font-medium px-2">Need-A Admin</div>
      ...
      <div className="flex flex-col gap-0.5">
        {navItems.map((item) => (
          <button
            ...
            className={cn(
              "text-left py-2.5 px-3 rounded-md text-[13px] cursor-pointer hover:bg-neutral-800",
              activeSection === item.key ? "bg-neutral-800 text-text" : "text-neutral-400",
            )}
          >
            {item.label}
          </button>
